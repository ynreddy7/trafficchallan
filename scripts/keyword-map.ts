...
// The nine state pages already in flight in a parallel task - not yet in
// loadStates(), but their keywords should be claimed now so they don't leak
// into the unassigned queue and spawn a duplicate/competing page.
const PLANNED_STATES: KnownPage[] = [
  { slug: 'uttar-pradesh', target_keyword: 'up e challan', type: 'state' },
  { slug: 'karnataka', target_keyword: 'karnataka e challan', type: 'state' },
  { slug: 'tamil-nadu', target_keyword: 'tn e challan', type: 'state' },
  { slug: 'andhra-pradesh', target_keyword: 'ap e challan', type: 'state' },
  { slug: 'haryana', target_keyword: 'haryana e challan', type: 'state' },
  { slug: 'rajasthan', target_keyword: 'rajasthan e challan', type: 'state' },
  { slug: 'gujarat', target_keyword: 'gujarat e challan', type: 'state' },
  { slug: 'west-bengal', target_keyword: 'west bengal e challan', type: 'state' },
  { slug: 'madhya-pradesh', target_keyword: 'mp e challan', type: 'state' }
];
...
function buildKnownPages(): KnownPage[] {
  ...
  return [...states, ...PLANNED_STATES, ...offences, ...guides, ...PLANNED_GUIDES];
}
...
function main() {
  const exportPath = process.argv[2] ?? join(process.cwd(), 'data', 'keywords', 'source', 'ubersuggest-traffic-challan-2026-08.csv');
  ...
  writeFileSync(join(outDir, 'keyword-map.json'), JSON.stringify(result, null, 2) + '\n');
