...
import { extractLocalHrefs, extractExternalHrefs, resolveToDistFile } from '../src/lib/linkcheck';

const UA = 'TrafficChallanBot/1.0 (+https://trafficchallan.com/about/)';
const checkExternalFlag = process.argv.includes('--external');

type ExternalResult = { url: string; status: 'alive' | 'warning' | 'dead'; detail: string };
...
async function checkExternal(url: string): Promise<ExternalResult> {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if (res.status === 403) return { url, status: 'warning', detail: '403 Forbidden (bot-blocked, treated as alive)' };
...
async function main() {
  ...
  if (checkExternalFlag) {
    ...
    for (const w of externalWarned) console.warn(`  WARNING external: ${w.url} — ${w.detail}`);
